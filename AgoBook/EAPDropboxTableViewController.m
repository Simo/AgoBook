//
//  EAPDropboxTableViewController.m
//  AgoBook
//
...
@implementation EAPDropboxTableViewController
...
- (void)reload
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^() {
        ...
        [updatedContents sortUsingFunction:sort context:NULL]; dispatch_async(dispatch_get_main_queue(), ^() {
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    __block EAPDropboxTableViewController *DropboxTVC = self;
...
- (IBAction)backup:(id)sender {
    ...
    if (account.isLinked) {
        CoreDataHelper *cdh =
        [(EAPAppDelegate *)[[UIApplication sharedApplication] delegate] cdh];
