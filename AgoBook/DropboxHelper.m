...
+ (NSURL*)renameLastPathComponentOfURL:(NSURL*)url toName:(NSString*)name
{
    ...
    NSError *error;
    [[NSFileManager defaultManager] moveItemAtPath:url.path toPath:newURL.path error:&error];
    
    if (error) {
        NSLog(@"ERROR renaming (i.e. moving) %@ to %@", url.lastPathComponent, newURL.lastPathComponent);
    } else {
...
+ (void)listFilesAtDropboxPath:(DBPath*)dropboxPath {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        NSError *error = nil;
        NSArray *contents = [[DBFilesystem sharedFilesystem] listFolder:dropboxPath error:&error];
        if (contents) {
...
+ (NSURL*)zipFolderAtURL:(NSURL*)url withZipfileName:(NSString*)zipFileName {
    ...
    NSFileManager *fileManager = [[NSFileManager alloc] init]; NSDirectoryEnumerator *directoryEnumerator =
    [fileManager enumeratorAtPath:url.path];
    // Write zip files for each file in the directory structure
    NSString *fileName;
    while (fileName = [directoryEnumerator nextObject]) {
        BOOL directory;
        NSString *filePath = [url.path stringByAppendingPathComponent:fileName]; [fileManager fileExistsAtPath:filePath isDirectory:&directory];
        if (!directory) {
            // get file attributes
            NSError *error = nil;
...
+ (void)unzipFileAtURL:(NSURL*)zipFileURL toURL:(NSURL*)unzipURL {
    @autoreleasepool {
        ...
        for (int i = 0; i < [unzipFile numFilesInZip]; i++) {
            ...
            NSMutableData *data = [[NSMutableData alloc] initWithLength:info.length]; [read readDataWithBuffer:data];
...
+ (void)restoreFromDropboxStoresZip:(NSString*)fileName withCoreDataHelper:(CoreDataHelper*)cdh {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:unzipFolder.path]) {
        ...
        if ([cdh reloadStore]) {
            [DropboxHelper deleteFileAtURL:oldBackupURL];
            UIAlertView *failAlert = [[UIAlertView alloc] initWithTitle:@"Restore Complete!" message:@"All data has been restored from the selected backup" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"Ok", nil];
            [failAlert show];
